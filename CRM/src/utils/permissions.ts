...
export const NAV_PERMISSIONS: Record<string, NavPermissionKey> = {
  ...
  '/online-appointments': 'viewOnlineAppointments',
...
export const PERMISSION_LABELS: { key: keyof Permissions; label: string; role: string }[] = [
  ...
  { key: 'viewOnlineAppointments', label: 'Online Appointments', role: 'Service Coordinator+' },
  { key: 'viewMyTasks', label: 'My Tasks', role: 'Technician' },
  { key: 'viewActivityLog', label: 'View Activity Log', role: 'Administrator only' },
...
export function normalizePermissions(permissions: Partial<Permissions> | undefined): Permissions {
  return {
    ...
    viewOnlineAppointments:
      permissions?.viewOnlineAppointments ?? permissions?.viewOnsiteAppointments ?? false,
    viewMyTasks: permissions?.viewMyTasks ?? false,
    viewActivityLog: permissions?.viewActivityLog ?? false,
